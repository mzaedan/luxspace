...
<section class="container mx-auto">
    <div class="flex flex-wrap my-4 md:my-12">
            ...
            <div class="slider">
                <div class="thumbnail">
                    <div class="px-2">
                        ...
                        <div class="item {{ $loop->first ? 'Selected' : ''}}" data-img="{{ asset('storage/'.str_replace('public', '', $item->url)) }}">
                            <img src="{{ asset('storage/'.str_replace('public', '', $item->url)) }}" alt="front" class="object-cover w-full h-full rounded-lg" />
                        </div>
                        @endforeach
                    </div>
                </div>
                <div class="preview">
                    <div class="item rounded-lg h-full overflow-hidden">
                        <img src="{{ $product->galleries()->exists() ? asset('storage/' . str_replace('public/', '', $product->galleries->first()->url)) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}" />
                    </div>
                </div>
            </div>
        </div>
        <div class="flex-1 px-4 md:p-6">
            <h2 class="text-5xl font-semibold">{{ $product->name }}</h2>
            <p class="text-xl" style="margin-top: 20px;">IDR {{ number_format($product->price) }}</p>

            <form action="{{ route('cart-add', $product->id) }}" method="POST">
                @csrf
                <button type="submit" class="transition-all duration-200 bg-pink-400 text-black focus:bg-black focus:text-pink-400 rounded-full px-8 py-3 mt-4 inline-flex"><svg class="fill-current mr-3" width="26" height="24" viewBox="0 0 26 24">
            ...
            <div class="px-3 flex-none" style="width: 320px">
                <div class="rounded-xl p-4 pb-8 relative bg-white">
                    <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                        <img src="{{ $recommendation->galleries()->exists() ? asset('storage/' . str_replace('public/', '', $recommendation->galleries->first()->url)) : asset('data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==') }}" />
                    </div>
